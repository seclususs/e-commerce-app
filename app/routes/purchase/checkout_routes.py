...
from app.services.orders.checkout_service import checkout_service # Import baru
...
def checkout():
    ...
    user_log_id = f"User {user_id}" if user_id else f"Session {session.get('session_id', 'N/A')}"

    if 'session_id' not in session and not user_id:
        session['session_id'] = str(uuid.uuid4())
        logger.info(f"Session_id baru dibuat untuk checkout tamu: {session['session_id']}")
    ...
    if request.method == 'POST':
        ...
        guest_cart_json = form_data.get('cart_data') if not user_id else None
        ...
        if result.get('redirect'):
            ...
            flash(result.get('message', 'Terjadi kesalahan.'), result.get('flash_category', 'danger'))
    ...
    try:
        ...
        if user_id:
            ...
            if not cart_details or not cart_details.get('items'):
                logger.warning(f"Pengguna {user_id} mengakses checkout dengan keranjang kosong.")
            ...
            if not hold_result['success']:
                logger.error(f"Gagal menahan stok untuk pengguna {user_id} saat checkout GET. Alasan: {hold_result['message']}")
                flash(hold_result['message'], 'danger')
                return redirect(url_for('purchase.cart_page'))

            stock_hold_expires = hold_result.get('expires_at')
            logger.info(f"Penahanan stok berhasil untuk pengguna {user_id} saat checkout GET. Kedaluwarsa pada: {stock_hold_expires}")

        else:
             logger.info(f"Menampilkan halaman checkout untuk pengguna tamu. ID Sesi: {session_id}. Validasi/penahanan stok akan dilakukan oleh frontend.")
        ...
        logger.error(f"Kesalahan memuat halaman checkout (GET) untuk {user_log_id}: {e}", exc_info=True)
...
def edit_address():
    ...
    if request.method == 'POST':
        ...
        logger.info(f"Pengguna {user_id} mengirim pembaruan alamat: {address_data}")

        try:
            result = user_service.update_user_address(user_id, address_data)
            if result['success']:
                flash('Alamat berhasil diperbarui.', 'success')
                logger.info(f"Alamat berhasil diperbarui untuk pengguna {user_id}.")
            ...
            logger.error(f"Kesalahan saat memperbarui alamat untuk pengguna {user_id} melalui rute checkout: {e}", exc_info=True)
    ...
    try:
        ...
        if not user:
            logger.error(f"Pengguna {user_id} tidak ditemukan saat mengakses halaman edit alamat.")
        ...
        logger.error(f"Kesalahan memuat halaman edit alamat untuk pengguna {user_id}: {e}", exc_info=True)
