...
def get_db() -> MySQLCursorDict:
    if "db" not in g:
        ...
        try:
            ...
            logger.error(
                f"Kesalahan saat menghubungkan ke MySQL: {err}", exc_info=True
            )
...
def close_db(e: Optional[Exception] = None) -> None:
    cursor: Optional[MySQLCursorDict] = g.pop("cursor", None)
    if cursor is not None:
        ...
        try:
            ...
            logger.error(f"Kesalahan saat menutup kursor: {ex}", exc_info=True)

    db: Optional[MySQLConnection] = g.pop("db", None)
    if db is not None:
        ...
        try:
            ...
            logger.error(
                f"Kesalahan saat menutup koneksi database: {err}", exc_info=True
            )

    elif e:
        logger.debug(
            f"close_db dipanggil dengan pengecualian, tetapi "
            f"tidak ditemukan koneksi DB aktif: {e}"
        )
...
def get_content() -> Dict[str, Any]:
    ...
    cursor: MySQLCursorDict = get_db()
    cursor.execute("SELECT `key`, `value` FROM content")
    content_data: List[Dict[str, Any]] = cursor.fetchall()
