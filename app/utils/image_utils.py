...
def save_compressed_image(
    ...
    if not allowed_file(file_storage.filename):
        ...
        raise InvalidFileTypeError(
            ...
            f"{file_storage.filename.rsplit('.', 1)[1].lower()}"
        )

    try:
        filename_base: str = str(uuid.uuid4())
        filename: str = f"{filename_base}.webp"
        ...
        logger.debug(
            f"Membuka stream gambar untuk: {file_storage.filename}"
        )

        if hasattr(image, "_getexif") and image._getexif() is not None:
            exif = dict(image._getexif().items())
            orientation_key: int = 274

            if orientation_key in exif:
                orientation: int = exif[orientation_key]
                logger.debug(
                    f"Gambar memiliki tag EXIF orientasi: {orientation}"
                )
        ...
        logger.info(
            f"Berhasil menyimpan gambar terkompresi sebagai: {filename}"
        )
        ...
        logger.error(
            f"Kesalahan memproses gambar {file_storage.filename}: {e}",
            exc_info=True,
        )
        raise FileUploadError(f"Gagal memproses unggahan gambar: {e}")