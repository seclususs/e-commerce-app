...
class ReviewService:
    ...
    def check_user_can_review(self, user_id, product_id, conn=None):
    ...
    def add_review(self, user_id, product_id, rating, comment):
        ...
        try:
            can_review = self.check_user_can_review(user_id, product_id, conn)
            if not can_review:
            ...
            conn.rollback()
            logger.error(f"Kesalahan DB saat menambahkan ulasan: {db_err}", exc_info=True)
            return {'success': False, 'message': 'Gagal menambahkan ulasan karena kesalahan database.'}
            ...
            return {'success': False, 'message': 'Gagal menambahkan ulasan karena kesalahan server.'}
