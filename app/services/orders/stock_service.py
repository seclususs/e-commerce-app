...
class StockService:
    """
    Layanan untuk mengelola logika stok, termasuk penahanan (stock hold).
    """

    def get_available_stock(self, product_id, variant_id=None, conn=None):
        """
        Menghitung stok yang tersedia (stok asli - stok yang ditahan).
        Juga membersihkan stok ditahan yang sudah kedaluwarsa.
        """
        close_conn = False
        if conn is None:
        ...
        try:
            # Hapus stok ditahan yang kedaluwarsa sebelum menghitung
            conn.execute("DELETE FROM stock_holds WHERE expires_at < CURRENT_TIMESTAMP")
            if not close_conn: # Jika koneksi dari luar, commit dulu
                 conn.commit()

            # Tentukan tabel mana yang akan di-query (products atau product_variants)
            ...
            if close_conn:
                conn.commit() # Commit pembersihan hold stock
                conn.close()

    def hold_stock_for_checkout(self, user_id, session_id, cart_items):
        """
        Membuat catatan penahanan stok selama 10 menit untuk item di keranjang checkout.
        """
        conn = get_db_connection()
        try:
            with conn:
                # Hapus hold lama untuk sesi atau pengguna ini
                if user_id:
                ...
                # Periksa ketersediaan stok untuk semua item sebelum menahan
                ...
                # Jika semua stok tersedia, buat hold baru
    ...
    def release_stock_holds(self, user_id, session_id, conn):
        """Melepas penahanan stok untuk user atau sesi tertentu menggunakan koneksi yang ada."""
        if user_id:
            conn.execute("DELETE FROM stock_holds WHERE user_id = ?", (user_id,))
        elif session_id:
            conn.execute("DELETE FROM stock_holds WHERE session_id = ?", (session_id,))
