...
class OrderService:
    ...
    def _prepare_items_for_order(self, conn, held_items):
        if not held_items:
            return None, None, {'success': False, 'message': 'Sesi checkout Anda telah berakhir. Silakan kembali ke keranjang.'}
        ...
        for item in held_items:
            ...
            if item['quantity'] > available_stock:
                 ...
                 return None, None, {'success': False, 'message': f"Stok untuk '{product['name']}'{size_info} telah habis saat akan memproses pesanan."}
    ...
    def _restock_order_items(self, order_id, conn):
        """Logic untuk mengembalikan stok item dari pesanan yang dibatalkan."""
    ...
    def update_order_status_and_tracking(self, order_id, new_status, tracking_number_input):
        ...
        try:
            with conn:
                ...
                tracking_number = tracking_number_input or order['tracking_number']
                ...
                if new_status == 'Dibatalkan' and order['status'] != 'Dibatalkan':
                    if order['status'] in ['Diproses', 'Dikirim']:
                        self._restock_order_items(order_id, conn)
