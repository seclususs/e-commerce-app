...
export function initGuestSubscribePage() {
    const layoutContainer = document.getElementById('guest-subscribe-layout-container');
    if (!layoutContainer) {
        return;
    }
    ...
    setTimeout(() => {
        layoutContainer.classList.add('show-form');
    }, transitionDelay);

    const form = document.getElementById('guest-subscribe-form');
    if (!form) return;
