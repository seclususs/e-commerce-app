...
with connection:
    ...
    cursor.execute("""
        CREATE TABLE users (
            ...
            province TEXT,
            postal_code TEXT,
            is_admin BOOLEAN NOT NULL DEFAULT 0, -- [DITAMBAHKAN] Kolom untuk admin
    ...
    # 3. Produk
    products_to_add = [
        ('Kaos "Hello, World!"', 175000, 'Kaos katun premium dengan sablon klasik "Hello, World!". Sempurna untuk memulai hari (atau proyek coding). Bahan adem dan nyaman.', 'T-Shirt', 'S, M, L, XL, XXL', 'Hitam, Putih, Biru Navy', 50, 'kaos_hello_world.webp', json.dumps(['kaos_hello_world_2.webp']), 100),
        ('Hoodie "Binary Tree"', 350000, 'Hoodie tebal dengan desain struktur data binary tree yang artistik. Jaga kehangatanmu saat begadang debugging. Fleece 280gsm.', 'Hoodie', 'M, L, XL', 'Hitam, Abu-abu', 85, 'hoodie_binary.webp', json.dumps([]), 50),
        ('Topi "Git Commit"', 125000, 'Topi baseball dengan bordir command "git commit". Sebuah pengingat untuk selalu menyimpan progresmu.', 'Aksesoris', 'All Size', 'Hitam', 70, 'topi_git.webp', json.dumps(['topi_git_2.webp']), 75),
        ('Kaos "404 Not Found"', 185000, 'Humor programmer dalam selembar kaos. Ketika kamu merasa hilang, setidaknya gayamu tetap ditemukan. Bahan katun 24s.', 'T-Shirt', 'S, M, L, XL', 'Putih', 95, 'kaos_404.webp', json.dumps([]), 80),
        ('Sweater "Code, Sleep, Repeat"', 295000, 'Sweater ringan yang cocok untuk kerja di ruangan ber-AC. Slogan yang mewakili gaya hidup setiap developer.', 'Sweater', 'M, L, XL, XXL', 'Biru Navy', 80, 'sweater_code.webp', json.dumps(['sweater_code_2.webp']), 60),
        ('T-Shirt "SQL Injection"', 175000, 'Sebuah lelucon keamanan siber yang hanya dimengerti oleh kalangan tertentu. Tunjukkan keahlianmu dengan gaya.', 'T-Shirt', 'L, XL', 'Hitam', 60, 'kaos_sql.webp', json.dumps([]), 40),
        ('Mug "Syntax Error"', 95000, 'Mulai harimu dengan kopi dan pengingat bahwa semua orang bisa membuat kesalahan. Keramik berkualitas tinggi.', 'Aksesoris', 'N/A', 'Putih', 90, 'mug_syntax.webp', json.dumps([]), 120),
        ('Sticker Pack Developer', 50000, 'Satu set stiker vinyl tahan air berisi logo bahasa pemrograman dan teknologi populer. Tempel di laptopmu!', 'Aksesoris', 'N/A', 'Multi-warna', 100, 'sticker_pack.webp', json.dumps([]), 200)
    ]
    cursor.executemany("""
        INSERT INTO products (name, price, description, category, sizes, colors, popularity, image_url, additional_image_urls, stock)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, products_to_add)
    ...
    # 4. Pesanan
    orders_to_add = [
        (1, 2, '2025-09-10 14:30:00', 525000, 'Completed', 'Bank Transfer', 'user', '081298765432', 'Jl. Merdeka No. 10', '', 'Depok', 'Jawa Barat', '16424'),
        (2, 2, '2025-10-01 11:00:00', 125000, 'Completed', 'E-Wallet', 'user', '081298765432', 'Jl. Merdeka No. 10', '', 'Depok', 'Jawa Barat', '16424'),
        (3, 3, '2025-10-14 20:00:00', 270000, 'Completed', 'COD', 'John Doe', '085611223344', 'Jl. Koding No. 42', '', 'Jakarta', 'DKI Jakarta', '12345')
    ]
    cursor.executemany("""
        INSERT INTO orders (id, user_id, order_date, total_amount, status, payment_method, shipping_name, shipping_phone, shipping_address_line_1, shipping_address_line_2, shipping_city, shipping_province, shipping_postal_code)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, orders_to_add)
